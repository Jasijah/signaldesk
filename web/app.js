...
const state = { view:'overview', domain:'All', query:'', caseId:null, caseTab:'overview', messageTab:'customer', cases:[], metrics:null, current:null };
...
async function load() {
  try {
    [state.cases,state.metrics]=await Promise.all([api('/api/cases'),api('/api/metrics')]);
...
function setNav() {
  ...
  document.getElementById('breadcrumb').textContent=state.view==='detail'?'Investigation':state.view==='new'?'New case':state.view==='analytics'?'Insights':state.view==='cases'?'Cases':'Overview';
...
function caseIcon(domain){const kind=domain==='Music'?'music':domain==='Service'?'service':'delivery';return `<span class="case-icon ${kind}">${icon(domain==='Music'?'music':domain==='Service'?'wrench':'truck')}</span>`;}
...
function renderOverview(){
  ...
  return heading('GOOD MORNING, JASIJAH','Your operations, in focus','A clear view of customer issues that need your attention.',`<button class="button subtle" data-action="reset">${icon('rotate')} Reset demo</button><button class="button primary" data-view="new">${icon('plus')} New case</button>`)+
    `<div class="metric-grid">${metric('Active cases',m.active,'Across all customer queues','blue','inbox')}${metric('Critical priority',m.critical,'Needs immediate attention','red','alert')}${metric('Resolved',m.resolved,'In this demo workspace','green','check')}${metric('Workflows',3,'Delivery, music + solo service','purple','activity')}</div>`+
    `<div class="panel-grid"><section class="panel"><div class="panel-header"><div><h2>Needs attention</h2><p>Prioritized partner investigations</p></div><button class="text-link" data-view="cases">View all ${icon('arrow')}</button></div>${state.cases.filter(c=>c.status!=='resolved').slice(0,3).map(row).join('')||'<div class="empty">All cases resolved.</div>'}</section>`+
    `<section class="panel"><div class="panel-header"><div><h2>Today’s focus</h2><p>One investigation at a time</p></div>${icon('sparkle')}</div><div class="focus-card"><span class="focus-overline">PRIORITY 01 · DELIVERY</span><h3>A completed delivery.<br>An outdated customer view.</h3><p>Follow the event trail to find where MetroMart’s status update stopped.</p><button class="button primary" data-case="SD-1042">Investigate case ${icon('arrow')}</button><div class="focus-stat"><span><strong>82</strong><small>orders affected</small></span><span><strong>504</strong><small>callback timeout</small></span></div></div><div class="scenario-list"><button class="scenario" data-case="SD-1043">${caseIcon('Music')}<span><strong>Explore music distribution</strong><small>Investigate a rejected release payload</small></span><span class="scenario-arrow">→</span></button><button class="scenario" data-case="SD-1046">${caseIcon('Service')}<span><strong>Explore a solo service desk</strong><small>Business SMS complaint and payment note</small></span><span class="scenario-arrow">→</span></button></div></section></div>`;
}
function renderCases(){
  const filtered=state.cases.filter(c=>(state.domain==='All'||c.domain===state.domain)&&(!state.query||`${c.title} ${c.account} ${c.id} ${c.subtitle}`.toLowerCase().includes(state.query.toLowerCase())));
  return heading('CASE WORKSPACE','Customer cases','Email, text, chat, social and partner alerts in one organized queue.',`<button class="button primary" data-view="new">${icon('plus')} New case</button>`)+
    `<div class="list-toolbar"><div class="segmented" aria-label="Filter by workflow">${['All','Delivery','Music','Service'].map(d=>`<button data-filter="${d}" class="${state.domain===d?'selected':''}">${d==='All'?'All cases':d}</button>`).join('')}</div><label class="search">${icon('search')}<input id="caseSearch" type="search" placeholder="Search cases or accounts" value="${escapeHTML(state.query)}" aria-label="Search cases"></label></div>`+
    `<section class="panel"><div class="panel-header"><div><h2>${state.domain==='All'?'All investigations':state.domain+' operations'}</h2><p>${filtered.length} fictional case${filtered.length===1?'':'s'} · live demo state</p></div></div>${filtered.map(c=>`<button class="full-case" data-case="${escapeHTML(c.id)}">${caseIcon(c.domain)}<span><h3>${escapeHTML(c.title)}</h3><p>${escapeHTML(c.account)} · ${escapeHTML(c.id)} · ${escapeHTML(c.channel||'Partner API')}</p></span><span class="case-meta">${badge(c.priority)}<small>${escapeHTML(c.status)} · ${escapeHTML(c.updated)}</small></span></button>`).join('')||'<div class="empty">No cases match this search.</div>'}</section>`;
}
...
function renderInsights(){const m=state.metrics,all=state.cases.length,percent=n=>Math.round(n/Math.max(all,1)*100);return heading('WORKFLOW INTELLIGENCE','A clearer view of the work','A small, honest snapshot of this fictional demo dataset.')+
  `<div class="metric-grid">${metric('Total cases',all,'Fictional seed records','blue','inbox')}${metric('Delivery queue',m.delivery,'Currently active','blue','truck')}${metric('Solo service',m.service,'Currently active','purple','wrench')}${metric('Resolved',m.resolved,'In this session','green','check')}</div>`+
  `<div class="insight-grid"><section class="panel"><div class="panel-header"><div><h2>Case mix</h2><p>Across three workflows</p></div></div><div class="section">${[['Delivery',''],['Music','purple'],['Service','']].map(([domain,color])=>`<div class="bar-row"><span>${domain}</span><div class="bar-track"><div class="bar-fill ${color}" style="width:${percent(state.cases.filter(c=>c.domain===domain).length)}%"></div></div><strong>${state.cases.filter(c=>c.domain===domain).length}</strong></div>`).join('')}</div></section><section class="panel"><div class="panel-header"><div><h2>What to look for</h2><p>Different intake, same clear follow-up</p></div></div><div class="section"><div class="insight-callout"><strong>Delivery:</strong> Compare an internal completion with the failed outbound callback.<br><br><strong>Music:</strong> Distinguish payload dispatch from partner acceptance.<br><br><strong>Solo service:</strong> Turn a phone complaint and a customer-reported payment into a trackable callback without claiming an unverified payment is confirmed.</div></div></section></div>`;}
function renderNew(){return `<div class="detail-top"><button class="back-button" data-view="cases">${icon('back')} All cases</button><span class="detail-id">UNIFIED INBOX DEMO</span></div>`+heading('SERVICE WORKFLOW','Capture a customer request','Keep complaints and follow-ups in a business workspace instead of a personal inbox.')+`<section class="panel"><form class="intake-form" id="intakeForm"><label>Customer name<input name="account" maxlength="200" minlength="2" placeholder="e.g. Avery Brooks" required></label><label>Where did it arrive?<select name="channel"><option>Email</option><option>SMS</option><option>Web chat</option><option>Social DM</option><option>Private form</option><option>Phone note</option></select></label><label>Service provided or requested<input name="service" maxlength="200" minlength="2" placeholder="e.g. Mobile phone screen repair" required></label><label>Customer message<textarea name="issue" maxlength="200" minlength="2" placeholder="Capture the request or complaint in their words" required></textarea></label><label>Payment context<select name="payment"><option>Not discussed</option><option>Unpaid</option><option>Customer reports paid</option><option>Owner verified paid</option></select></label><p class="form-hint">This local demo simulates channel intake. It does not receive real texts, email, chats, or social DMs, or verify Cash App payments. A production version would connect a dedicated business number and inbox.</p><button class="button primary" type="submit">${icon('plus')} Create case</button></form><div class="side-section"><strong style="font-size:12px">Move complaints off social media</strong><p class="diagnostic-intro" style="margin:8px 0">Reply with a private request link so the customer can explain the issue away from a public comment thread.</p><a class="button" href="/intake.html" target="_blank" rel="noopener">Preview customer form →</a></div></section>`;}
function render(){setNav();content.innerHTML=state.view==='detail'?renderDetail():state.view==='cases'?renderCases():state.view==='new'?renderNew():state.view==='analytics'?renderInsights():renderOverview();}
